...
require_once("../conexion.php");
    ...
    function mostrarAlerta($titulo, $mensaje, $urlDestino = 'empleados.php') {
        echo '<!DOCTYPE html>
        <html>
        <head>
            <meta charset="utf-8">
...
if (isset($_GET['id'])) {
    ...
    $sql_check = "SELECT 
                    (SELECT COUNT(*) FROM venta WHERE Cod_Empleado = $id_empleado) + 
                    (SELECT COUNT(*) FROM intercambio WHERE Cod_Empleado = $id_empleado) AS total_registros";
    ...
    if ($total_registros == 0) {
        $sql_delete = "DELETE FROM empleado WHERE Cod_Empleado = $id_empleado";
        if ($conn->query($sql_delete) === TRUE) {
            mostrarAlerta('Empleado Eliminado', 'El empleado ha sido eliminado permanentemente del sistema.');
        } else {
            mostrarAlerta('Error', 'No se pudo eliminar al empleado: ' . $conn->error);
        }
    } else {
        $sql_update = "UPDATE empleado SET Activo = 0 WHERE Cod_Empleado = $id_empleado";
        if ($conn->query($sql_update) === TRUE) {
            mostrarAlerta('¡Baja Exitosa!', 'El empleado tiene un historial de movimientos, por lo que ha sido dado de baja.');
        } else {
            mostrarAlerta('Error', 'Hubo un error al intentar dar de baja: ' . $conn->error);
        }
    }
} else {
    mostrarAlerta('Acceso Denegado', 'No se especificó ningún empleado válido.');
}
?>