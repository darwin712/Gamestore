...
<html>
    ...
    <body>
        ...
        <section id="background">
            <div id="title"> Historial de Ventas </div>

            <div id="topBar">
                <form action="historialVentas.php" method="POST" id="formFiltros">
            ...
            <div id="scroll2">
                ...
                <?php
                    $sql = "SELECT 
                                ...
                                GROUP_CONCAT(CONCAT(p.Nombre, ' (x', dv.Cantidad, ')') SEPARATOR '<br>') AS Detalles
                            FROM venta v
                    ...
                    if (isset($_POST['busqueda']) && $_POST['busqueda'] != "") {
                        $idVenta = intval($_POST['busqueda']);
                        $sql .= " AND v.Cod_Venta = $idVenta";
                    }

                    if (isset($_POST['empleado']) && $_POST['empleado'] != "") {
                        $idEmp = intval($_POST['empleado']);
                        $sql .= " AND v.Cod_Empleado = $idEmp";
                    }

                    if (isset($_POST['precioMin']) && $_POST['precioMin'] != "") {
                        $precioMin = floatval($_POST['precioMin']);
                        $sql .= " AND v.Total >= $precioMin";
                    }

                    if (isset($_POST['precioMax']) && $_POST['precioMax'] != "") {
                        $precioMax = floatval($_POST['precioMax']);
                        $sql .= " AND v.Total <= $precioMax";
                    }

                    $sql .= " GROUP BY v.Cod_Venta";

                    if (isset($_POST['orden']) && $_POST['orden'] != "") {
                        switch ($_POST['orden']) {
                            case "recientes":
                                $sql .= " ORDER BY v.Fecha DESC, v.Cod_Venta DESC";
                                break;
                            case "antiguas":
                                $sql .= " ORDER BY v.Fecha ASC, v.Cod_Venta ASC";
                                break;
                            case "mayorMonto":
                ...
                ?>
                            <tr>
                                <td><b>#<?php echo htmlspecialchars($row['Cod_Venta']); ?></b></td>
                ...
                <?php
                        ...
                        echo "<p style='margin-top:20px;'>No se encontraron registros de ventas que coincidan con tu búsqueda.</p>";
                    }
                ?>
            </div>
        </section>
